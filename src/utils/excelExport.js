...
export const exportMonthlyToExcel = ({
  ...
  transactions.forEach((t) => {
    ...
    transactionMap[t.memberId][t.date] =
      (transactionMap[t.memberId][t.date] || 0) + t.amount;
  });
  ...
  const overallMonthlyTotal = Object.values(memberMonthlyTotals).reduce(
    (sum, val) => sum + val,
    0
  );
  ...
  members.forEach((member) => {
    const balance =
      memberMonthlyTotals[member.id] - (member.monthlyTarget || 0);
 ...
 * @param {string} params.monthYear - Month in YYYY-MM format
 */
export const exportMemberMonthlyToExcel = ({
  member,
  transactions,
  allTransactions,
  monthYear,
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    data.push([date, dayName, amount > 0 ? amount : "", status]);
  }
  ...
  const totalPaidAllTime = allTransactions.reduce(
    (sum, t) => sum + t.amount,
    0
  );
  ...
  data.push([
    "Status",
    cumulativeBalance >= 0 ? "Paid Full/Credit" : "Balance Due",
  ]);
  ...
  data.push([
    "Payment Rate",
    `${Math.round((daysWithPayment / totalDays) * 100)}%`,
  ]);
  ...
  // Generate filename
  const filename = `${member.name.replace(/\s+/g, "_")}_${monthName.replace(
    " ",
    "_"
  )}.xlsx`;
